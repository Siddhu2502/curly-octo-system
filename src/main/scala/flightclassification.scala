...
writer.append("Num of flights per origin terminal \n")
...
writer.append("Num of flights per destination \n")
...
val avgArrivalDelaysPerAirport = flightsRDD.map(flight => (flight.origin, flight.crselapsedtime)).groupByKey().mapValues(avg => avg.sum/avg.size)
println("\nAverage departure delay per airport: \n")
println("Key\tValue")
...
val arrival_delay_count = flightsRDD.filter(flight => flight.crselapsedtime > 20).count()
...
val delayed_flight_atdepandarr = flightsRDD.filter(flight => flight.depdelaymins > 20 && flight.crselapsedtime > 20).count()
...
val delayed_flight_atdepandarr = flightsRDD.filter(flight => flight.depdelaymins == 0 && flight.crselapsedtime == 20).count()
...
val top_departure_delay = flightsRDD.map(flight => (flight.dest, flight.crselapsedtime)).reduceByKey(_ + _).sortBy(_._2, false).take(3)
...
val departure_delay_df = top_departure_delay.map { case (code, delay) =>
  ...
  val delayed_flights = flightsRDD.filter(flight => flight.dest == code && flight.crselapsedtime > 0).count()
  (code, total_flights, delayed_flights)
}
...
val top_departure_ontime = flightsRDD.map(flight => (flight.dest, flight.crselapsedtime)).reduceByKey(_ + _).sortBy(_._2, false).take(3)
val departure_ontime_df = top_departure_ontime.map { case (code, delay) =>
  val total_flights = flightsRDD.filter(flight => flight.dest == code).count()
  val ontime_flights = flightsRDD.filter(flight => flight.dest == code && flight.crselapsedtime == 0).count()
  (code, total_flights, ontime_flights)
}
...
val top_planes_highest_arrival_delays = flightsRDD.map(flight => (flight.tailnum, flight.crselapsedtime)).groupByKey().mapValues(avg => avg.sum/avg.size.toDouble).sortBy(_._2, false).take(3)
...
val top_planes_lowest_arrival_delays = flightsRDD.map(flight => (flight.tailnum, flight.crselapsedtime)).groupByKey().mapValues(avg => avg.sum/avg.size.toDouble).sortBy(_._2).take(3)
